class Constant

  # Constants
  EN_TRANSLATION_PENDING = "pending translation"
  ES_TRANSLATION_PENDING = "traduccion pendiente"

  ALBUMS_DEFAULT_VALUES = ["Profile Photo", "Professional Book", "Polaroid"]

  PEOPLE_CAN_VOTE = ["contractor", "profile_model", "profile_photographer"]

  CASTING_CREATION_COST = 30
  CASTING_UPDATE_DATES_CHANGE_COST = 30

  BOOKING_CREATION_COST = 30
  BOOKING_UPDATE_DATES_CHANGE_COST = 30

  ALBUM_PROFILE_NAME = "Profile Photo"
  ALBUM_PROFESSIONAL_NAME = "Professional Book"
  ALBUM_POLAROID_NAME = "Polaroid"

  CASTING_MIN_PAYMENT_PER_MODEL_VALUE = 100
  CASTING_RECOMMENDED_PAYMENT_PER_MODEL_VALUE = 500
  CASTING_NO_PAYMENT_PER_MODEL_TEXT = "TFP"

  PLAN_BASIC_TEXT = "basic"
  PLAN_PREMIUM_TEXT = "premium"
  
end